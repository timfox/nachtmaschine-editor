...
- **Maya ASCII level import:** **File → Import / Reload / Unload Maya Scene…** (`mdl/LoadMayaAsciiScene.*`). Tags: `tb_entity_*` / `tb_spawn_*` (points), `tb_model_*` / `tb_model` (models only), `tb_trigger_*` / `tb_brush_*` (brush entities; mesh `.vt` → convex hull with AABB fallback), `tb_brush_hull_*` / `tb_brush_box_*`, `tb_locator_*`, plus `tb_class`, `tb_material`, `tb_kind`, `tb_brush_shape` (`box`/`hull`). Untagged mesh is skipped. MEL: parent repo **`tools/maya/nacht_tb_tag_selection.mel`**. Maya Y-up → TrenchBroom Z-up.
